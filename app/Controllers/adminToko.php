<?php 
namespace App\Controllers;

class adminToko extends BaseController {
	public $admin_model,$db,$session,$uri,$c, $data;

	public function __construct() {
        $this->db = \Config\Database::connect('homeToko');
		$this->admin_model = new \App\Models\admin_model();
		$this->uri = new \CodeIgniter\HTTP\URI();
        $this->data['logo'] = $this->admin_model->GetLogo()->getRowArray();
        $this->data['status'] = $this->db->query("select count(status) as stts from tbl_hubungikami where status='0'")->getRowArray();
        helper('form');
		$this->data['status_transaksi'] = $this->db->query("select count(status) as stts from tbl_transaksi_header where status='0'")->getRowArray();   		
		$this->uri = new \CodeIgniter\HTTP\URI();

		// thirdparty login
		$this->session = service('session'); 	
		$this->auth = service('authentication');
	}
// Tahap 1 halaman home landing page Login
	public function index() {
        $data['logo'] = $this->admin_model->GetLogo()->getResultArray();
        echo view('adminweb/login',$data);
	}
// Tahap 2 Cek login berhasil atau tidak
	public function login() { 
		if(!$this->validate([
			'username' => 'required',
			'password' => 'required'
		])){
			// jika password dan username kosong maka akan memunculkan peringatan
			$data['validation'] = \Config\Services::validation();
			echo 'password dan username belum di isi';
			dd($data['validation']);
		}else{
			$username = $_POST['username'];
			$password = $_POST['password'];
			$cek = $this->admin_model->CekAdminLogin($username,$password);
			// dd($cek);
			// !$this->auth->check()
			if ( $cek == true )
			{
				$redirectURL = session('redirect_url') ?? '/';
				unset($_SESSION['redirect_url']);
	
				return redirect()->to($redirectURL.'adminToko/home');
			}else{
				return redirect()->to($redirectURL.'adminToko');
			}


		}

    }
// tahap 3 halaman masuk Toko ke halamman dasboard
	public function home() {
        $data['logo'] = $this->admin_model->GetLogo()->getRowArray();
		$data['status'] = $this->admin_model->getCekStatus()->getRowArray();
        $data['status_transaksi'] = $this->admin_model->getCekStatusTransaksi()->getRowArray();
        echo view('adminweb/template/header', $data);
        echo view('adminweb/home');
        echo view('adminweb/template/footer', $data);
	}

	public function logout() {
		$this->session->sess_destroy();
		redirect("adminweb");
	} 

	// Awal dari controller untuk transaksi ---------------------------------------------------------------
	public function transaksi() {

		// if($this->session->userdata("logged_in")!=="") {
            if(true){
        //    data_header();
			$data['data_transaksi'] = $this->admin_model->GetTransaksi()->getResultArray();
            $data = $data + $this->data;
            echo view('adminweb/template/header', $data);
            echo view('adminweb/transaksi/index',$data);
            echo view('adminweb/template/footer', $data);     
		}
		else {
			redirect("adminweb");
		}

	}

	public function transaksi_proses () {

		// if($this->session->userdata("logged_in")!=="") {
            if(true){

			$id  = $this->request->uri->getSegment(3);

			$this->admin_model->UpdateTransaksiHeader($id);

			// $this->session->set_flashdata('berhasil','Transaksi Berhasil Di Proses');
			// redirect("adminweb/transaksi");
			return redirect()->to('/adminToko/transaksi/');

		}
		else {
			redirect("adminweb");
		}

	}

	public function transaksi_detail () {

		// if($this->session->userdata("logged_in")!=="") {
            if(true){

			$id  			= $this->request->uri->getSegment(3);
			$kode_transaksi = $this->request->uri->getSegment(4);

			$data['data_header'] 	= $this->admin_model->GetTransaksiheader($id)->getResultArray();  
			$data['data_detail']	= $this->admin_model->GetDetailTransaksi($kode_transaksi)->getResultArray();
			$data['data_total']		= $this->admin_model->GetDetailTotal($kode_transaksi)->getResultArray();

           $data = $data + $this->data; 
            echo view('adminweb/template/header', $data);
            echo view('adminweb/transaksi/detail',$data);
            echo view('adminweb/template/footer', $data);

		}
		else {
			redirect("adminweb");
		}

	}

	public function semua_transaksi () {

		// if($this->session->userdata("logged_in")!=="") {
            if(true){

			$data['data_transaksi'] = $this->admin_model->GetTransaksiSudah()->getResultArray();

           $data = $data + $this->data; 
            echo view('adminweb/template/header', $data);
            echo view('adminweb/transaksi/sudah',$data);
            echo view('adminweb/template/footer', $data);


		}
		else {
			redirect("adminweb");
		}

	}

	public function semua_transaksi_detail () {

		// if($this->session->userdata("logged_in")!=="") {
            if(true){

			$id  			= $this->request->uri->getSegment(3);
			$kode_transaksi = $this->request->uri->getSegment(4);

			$data['data_header'] 	= $this->admin_model->GetTransaksiheader($id)->getResultArray();  
			$data['data_detail']	= $this->admin_model->GetDetailTransaksi($kode_transaksi)->getResultArray();
			$data['data_total']		= $this->admin_model->GetDetailTotal($kode_transaksi)->getResultArray();

           $data = $data + $this->data; 
            echo view('adminweb/template/header', $data);
            echo view('adminweb/transaksi/detail_semua',$data);
            echo view('adminweb/template/footer', $data);

		}
		else {
			redirect("adminweb");
		}

	}


	//Awal Seo
	public function seo() {
		// if ($this->session->userdata("logged_in")!=="") {
            if(true){
			$query = $this->admin_model->GetSeo()->getResultArray();
			foreach ($query as $tampil) {
				$data['id_seo']=$tampil['id_seo'];
				$data['tittle']=$tampil['tittle'];
				$data['keyword']=$tampil['keyword'];
				$data['description']=$tampil['description'];
			}
           $data = $data + $this->data; 
            echo view('adminweb/template/header', $data);
            echo view('adminweb/seo/index',$data);
            echo view('adminweb/template/footer', $data);
		}
		else {
			redirect("adminweb");
		}
	}

	public function seo_simpan() {
		$id_seo = $_POST['id_seo'];
		$tittle = $_POST['tittle'];
		$keyword = $_POST['keyword'];
		$description = $_POST['description'];

		$this->admin_model->UpdateSeo($id_seo,$tittle,$keyword,$description);
	}
	//Akhir Seo

	//Awal Kategori Galeri
	public function kategorigaleri() {

		$data['data_kategorigaleri']= $this->admin_model->Getkategorigaleri()->getRowArray();
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/kategorigaleri/index',$data);
            echo view('adminweb/template/footer', $data);
	}

	public function kategorigaleri_simpan() {
		$nama_kategorigaleri = $_POST["nama_kategorigaleri"];
		$cek = $this->admin_model->kategorigaleriSama($nama_kategorigaleri);

		if ($cek->num_rows()>0) {
			$success = "1";
		}
		else {
			$this->session->set_flashdata('berhasil','kategorigaleri Berhasil Disimpan');
			$this->admin_model->kategorigaleriSimpan($nama_kategorigaleri);
			$success="0";
		}

		echo $success;
	}

	public function kategorigaleri_edit() {
		$id_kategorigaleri = $this->request->uri->getSegment(3);
		$query = $this->admin_model->GetEditkategorigaleri($id_kategorigaleri);
		foreach ($query->result_array() as $tampil) {
			$data['id_kategorigaleri'] = $tampil['id_kategorigaleri'];
			$data['nama_kategorigaleri'] = $tampil['nama_kategorigaleri'];
		}

       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/kategorigaleri/edit',$data);
            echo view('adminweb/template/footer', $data);
	}

	public function kategorigaleri_delete() {
		$id_kategorigaleri = $this->request->uri->getSegment(3);
		$this->admin_model->Deletekategorigaleri($id_kategorigaleri);

		$this->session->set_flashdata('message','kategorigaleri Berhasil Dihapus');
		redirect("adminweb/kategorigaleri");
	}

	public function kategorigaleri_update() {
		$id_kategorigaleri = $_POST['id_kategorigaleri'];
		$nama_kategorigaleri = $_POST['nama_kategorigaleri'];

		$cek = $this->admin_model->kategorigaleriSama($nama_kategorigaleri);

		if ($cek->num_rows()>0) {
			$success = "1";
		}
		else {
			$this->session->set_flashdata('berhasil','kategorigaleri Berhasil Disimpan');
			$this->admin_model->kategorigaleriUpdate($id_kategorigaleri,$nama_kategorigaleri);
			$success="0";
		}
 
		echo $success;
	}
	//Akhir kategori

	//Awal Galeri
	public function galeri() {

		$data['data_galeri'] = $this->admin_model->GetGaleri();
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/galeri/index',$data);
            echo view('adminweb/template/footer', $data);

	}

	public function galeri_tambah() {
		$data['data_kategorigaleri'] = $this->admin_model->Getkategorigaleri();
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/galeri/add',$data);
            echo view('adminweb/template/footer', $data);
	}

	public function galeri_simpan() {

			
			$this->form_validation->set_rules('kategorigaleri_id', 'Album', 'required');
			$this->form_validation->set_rules('nama_galeri', 'Nama Gallery', 'required');
			
		
			

			if ($this->form_validation->run() == FALSE)
			{
				$data['data_kategorigaleri']= $this->admin_model->Getkategorigaleri();
               $data = $data + $this->data; 
                echo view('adminweb/template/header', $data);
                echo view('adminweb/galeri/add',$data);
            echo view('adminweb/template/footer', $data);
			}
			else {

				if(empty($_FILES['userfile']['name']))
				{
					$in_data['nama_galeri'] = $_POST['nama_galeri'];
					$in_data['kategorigaleri_id'] = $_POST['kategorigaleri'];
					$this->db->insert("tbl_galeri",$in_data);

					$this->session->set_flashdata('berhasil','Gallery Berhasil Disimpan');
					redirect("adminweb/galeri");
				}
				else
				{
					$config['upload_path'] = './images/galeri/';
					$config['allowed_types']= 'gif|jpg|png|jpeg';
					$config['encrypt_name']	= TRUE;
					$config['remove_spaces']	= TRUE;	
					$config['max_size']     = '3000';
					$config['max_width']  	= '3000';
					$config['max_height']  	= '3000';
					
			 
					$this->load->library('upload', $config);
	 
					if ($this->upload->do_upload("userfile")) {
						$data	 	= $this->upload->data();
			 
						/* PATH */
						$source             = "./images/galeri/".$data['file_name'] ;
						$destination_thumb	= "./images/galeri/thumb/" ;
						$destination_medium	= "./images/galeri/medium/" ;
						// Permission Configuration
						chmod($source, 0777) ;
			 
						/* Resizing Processing */
						// Configuration Of Image Manipulation :: Static
						$this->load->library('image_lib') ;
						$img['image_library'] = 'GD2';
						$img['create_thumb']  = TRUE;
						$img['maintain_ratio']= TRUE;
			 
						/// Limit Width Resize
						$limit_medium   = 800 ;
						$limit_thumb    = 270 ;
			 
						// Size Image Limit was using (LIMIT TOP)
						$limit_use  = $data['image_width'] > $data['image_height'] ? $data['image_width'] : $data['image_height'] ;
			 
						// Percentase Resize
						if ($limit_use > $limit_thumb) {
							$percent_medium = $limit_medium/$limit_use ;
							$percent_thumb  = $limit_thumb/$limit_use ;
						}
			 
						//// Making THUMBNAIL ///////
						$img['width']  = $limit_use > $limit_thumb ?  $data['image_width'] * $percent_thumb : $data['image_width'] ;
						$img['height'] = $limit_use > $limit_thumb ?  $data['image_height'] * $percent_thumb : $data['image_height'] ;
			 
						// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_thumb ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;

						$img['width']   = $limit_use > $limit_medium ?  $data['image_width'] * $percent_medium : $data['image_width'] ;
						$img['height']  = $limit_use > $limit_medium ?  $data['image_height'] * $percent_medium : $data['image_height'] ;
			 
			 			// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_medium ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;
						
						$in_data['nama_galeri'] = $_POST['nama_galeri'];
						$in_data['gambar'] = $data['file_name'];
						$in_data['kategorigaleri_id'] = $_POST['kategorigaleri_id'];
						
						
						$this->db->insert("tbl_galeri",$in_data);

				
						
						$this->session->set_flashdata('berhasil','Gallery Berhasil Disimpan');
						redirect("adminweb/galeri");
						
					}
					else 
					{
                        
                       $data = $data + $this->data; echo view('adminweb/galeri/error');
                        echo view('adminweb/template/header', $data);
                        echo view('adminweb/template/footer', $data);
                    }
				}
				
			}

	}

	public function galeri_delete() {
		$id_galeri = $this->request->uri->getSegment(3);
		$this->admin_model->DeleteGaleri($id_galeri);

		$this->session->set_flashdata('message','Gallery Berhasil Dihapus');
		redirect('adminweb/galeri');

	}

	public function galeri_edit() {
		$id_galeri = $this->request->uri->getSegment(3);
		$query = $this->admin_model->GetGaleriEdit($id_galeri);
		foreach ($query->result_array() as $tampil) {
			$data['id_galeri'] = $tampil['id_galeri'];
			$data['nama_galeri'] = $tampil['nama_galeri'];
			$data['gambar'] = $tampil['gambar'];
			$data['kategorigaleri_id'] = $tampil['kategorigaleri_id'];
		}
		$data['data_kategorigaleri'] = $this->admin_model->Getkategorigaleri();
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/galeri/edit',$data);
            echo view('adminweb/template/footer', $data);
	}

	public function galeri_update() {
		$id['id_galeri'] = $_POST["id_galeri"];

		if(empty($_FILES['userfile']['name']))
				{
					
						$in_data['nama_galeri'] = $_POST['nama_galeri'];
						$in_data['kategorigaleri_id'] = $_POST['kategorigaleri_id'];
						
						$this->db->update("tbl_galeri",$in_data,$id);

					$this->session->set_flashdata('update','Gallery Berhasil Diupdate');
					redirect("adminweb/galeri");
				}
				else
				{
					$config['upload_path'] = './images/galeri/';
					$config['allowed_types']= 'gif|jpg|png|jpeg';
					$config['encrypt_name']	= TRUE;
					$config['remove_spaces']	= TRUE;	
					$config['max_size']     = '3000';
					$config['max_width']  	= '3000';
					$config['max_height']  	= '3000';
					
			 
					$this->load->library('upload', $config);
	 
					if ($this->upload->do_upload("userfile")) {
						$data	 	= $this->upload->data();
			 
						/* PATH */
						$source             = "./images/galeri/".$data['file_name'] ;
						$destination_thumb	= "./images/galeri/thumb/" ;
						$destination_medium	= "./images/galeri/medium/" ;
			 
						// Permission Configuration
						chmod($source, 0777) ;
			 
						/* Resizing Processing */
						// Configuration Of Image Manipulation :: Static
						$this->load->library('image_lib') ;
						$img['image_library'] = 'GD2';
						$img['create_thumb']  = TRUE;
						$img['maintain_ratio']= TRUE;
			 
						/// Limit Width Resize
						$limit_medium   = 800 ;
						$limit_thumb    = 270 ;
			 
						// Size Image Limit was using (LIMIT TOP)
						$limit_use  = $data['image_width'] > $data['image_height'] ? $data['image_width'] : $data['image_height'] ;
			 
						// Percentase Resize
						if ($limit_use > $limit_thumb) {
							$percent_medium = $limit_medium/$limit_use ;
							$percent_thumb  = $limit_thumb/$limit_use ;
						}
			 
						//// Making THUMBNAIL ///////
						$img['width']  = $limit_use > $limit_thumb ?  $data['image_width'] * $percent_thumb : $data['image_width'] ;
						$img['height'] = $limit_use > $limit_thumb ?  $data['image_height'] * $percent_thumb : $data['image_height'] ;
			 
						// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_thumb ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;
	 
						////// Making MEDIUM /////////////
						$img['width']   = $limit_use > $limit_medium ?  $data['image_width'] * $percent_medium : $data['image_width'] ;
						$img['height']  = $limit_use > $limit_medium ?  $data['image_height'] * $percent_medium : $data['image_height'] ;
			 
						// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_medium ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;
						
						$in_data['nama_galeri'] = $_POST['nama_galeri'];
						$in_data['gambar'] = $data['file_name'];
						$in_data['kategorigaleri_id'] = $_POST['kategorigaleri'];
						
						$this->db->update("tbl_galeri",$in_data,$id);
				
						
						$this->session->set_flashdata('update','Gallery Berhasil Diupdate');
						redirect("adminweb/galeri");
						
					}
					else 
					{
                        
                       $data = $data + $this->data; echo view('adminweb/galeri/error');
                        echo view('adminweb/template/header', $data);
                        echo view('adminweb/template/footer', $data);
                    }
				}

	}
	//Akhir Galeri

	//Awal Logo
	public function logo () {
		$query = $this->admin_model->GetLogo()->getResultArray();
		foreach ($query as $tampil) {
			$data['id_logo']=$tampil['id_logo'];
			$data['gambar']=$tampil['gambar'];
		}
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/logo/index',$data);
            echo view('adminweb/template/footer', $data);
	}

	 public function logo_simpan()
   {
		// if($this->session->userdata("logged_in")!=="") {
            if(true){
			$id['id_logo'] = $_POST["id_logo"];
			$id_logo = $_POST["id_logo"];
			

				if(empty($_FILES['userfile']['name']))
				{
					
					
					$this->session->set_flashdata('message','Logo Berhasil Diupdate');
					redirect("adminweb/logo");
				}
				else
				{
					$config['upload_path'] = './images/logo/';
					$config['allowed_types']= 'gif|jpg|png|jpeg';
					$config['encrypt_name']	= TRUE;
					$config['remove_spaces']	= TRUE;	
					$config['max_size']     = '3000';
					$config['max_width']  	= '500';
					$config['max_height']  	= '250';
			 
					$this->load->library('upload', $config);
	 
					if ($this->upload->do_upload("userfile")) {
						$data	 	= $this->upload->data();
			 
						/* PATH */
						$source             = "./images/logo/".$data['file_name'] ;
						$destination_thumb	= "./images/logo/thumb/" ;
			 
						// Permission Configuration
						chmod($source, 0777) ;
			 
						/* Resizing Processing */
						// Configuration Of Image Manipulation :: Static
						$this->load->library('image_lib') ;
						$img['image_library'] = 'GD2';
						$img['create_thumb']  = TRUE;
						$img['maintain_ratio']= TRUE;
			 
						/// Limit Width Resize
						$limit_thumb    = 640 ;
			 
						// Size Image Limit was using (LIMIT TOP)
						$limit_use  = $data['image_width'] > $data['image_height'] ? $data['image_width'] : $data['image_height'] ;
			 
						// Percentase Resize
						if ($limit_use > $limit_thumb) {
							$percent_thumb  = $limit_thumb/$limit_use ;
						}
			 
						//// Making THUMBNAIL ///////
						$img['width']  = $limit_use > $limit_thumb ?  $data['image_width'] * $percent_thumb : $data['image_width'] ;
						$img['height'] = $limit_use > $limit_thumb ?  $data['image_height'] * $percent_thumb : $data['image_height'] ;
			 
						// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_thumb ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;
						
						$in_data['gambar'] = $data['file_name'];
						$this->db->update("tbl_logo",$in_data,$id);
				
						
						$this->session->set_flashdata('message','Logo Berhasil Diupdate');
						redirect("adminweb/logo");
						
					}
					else 
					{
						echo $this->upload->display_errors('<p>','</p>');
					}
				}
			
		}
		else
		{
			redirect("adminweb");
		}
   }
	//Akhir Logo

   //Awal kontak
	public function kontak() {
		// if($this->session->userdata("logged_in")!=="") {
            if(true){

			$query=$this->admin_model->Getkontak()->getResultArray();
			foreach ($query as $tampil) {
				$data["id_kontak"]=$tampil["id_kontak"];
				$data["alamat"]=$tampil["alamat"];
				$data["phone"]=$tampil["phone"];
				$data["email"]=$tampil["email"];
			}

           $data = $data + $this->data; 
            echo view('adminweb/template/header', $data);
            echo view('adminweb/kontak/index',$data);
            echo view('adminweb/template/footer', $data);
		}
		else {
			redirect("adminweb");
		}
	}

	public function kontak_simpan() {
		$id_kontak =$_POST["id_kontak"];
		$alamat =$_POST["alamat"];
		$phone =$_POST["phone"];
		$email =$_POST["email"];

		$this->admin_model->Simpankontak($id_kontak,$alamat,$phone,$email);
	}
	//Akhir kontak

	//Awal Sosial Media 
   public function sosial_media() {
   	// if($this->session->userdata("logged_in")!=="") {
           if(true){
		   	$query = $this->admin_model->GetSosialMedia()->getResultArray();
		   	foreach ($query as $tampil) {
		   		$data['id_sosial_media'] = $tampil['id_sosial_media'];
		   		$data['tw'] = $tampil['tw'];
		   		$data['fb'] = $tampil['fb'];
		   		$data['gp'] = $tampil['gp'];
		   	}
           $data = $data + $this->data;
               echo view('adminweb/template/header', $data);
               echo view('adminweb/sosial_media/index',$data);
            echo view('adminweb/template/footer', $data);
	}
	else {
		redirect("adminweb");
	}
   }

   public function sosial_media_simpan() {
		$id_sosial_media =$_POST["id_sosial_media"];
		$tw =$_POST["tw"];
		$fb =$_POST["fb"];
		$gp =$_POST["gp"];
		

		$this->admin_model->SimpanSosialMedia($id_sosial_media,$tw,$fb,$gp);
	}
   //Akhir Sosial Media

	//Awal Kategori
	public function kategori() {

		$data['data_kategori']= $this->admin_model->GetKategori()->getResultArray();
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/kategori/index',$data);
            echo view('adminweb/template/footer', $data);
	}

	public function kategori_simpan() {
		$nama_kategori = $_POST["nama_kategori"];
		// $nama_kategori = 'ok';
		$cek = $this->admin_model->KategoriSama($nama_kategori)->getResultArray();

		// if (count()>0) {
		if (count($cek)>0) {
			$success = "1";
		}
		else {
			// $this->session->set_flashdata('berhasil','Kategori Berhasil Disimpan');
			$this->admin_model->KategoriSimpan($nama_kategori);
			$success="0";
		}

		echo $success;
	}

	public function kategori_edit() {
		$id_kategori = $this->request->uri->getSegment(3);
		$query = $this->admin_model->GetEditKategori($id_kategori)->getResultArray();
		foreach ($query as $tampil) {
			$data['id_kategori'] = $tampil['id_kategori'];
			$data['nama_kategori'] = $tampil['nama_kategori'];
		}

       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/kategori/edit',$data);
        echo view('adminweb/template/footer', $data);
	}

	public function kategori_delete() {
		$id_kategori = $this->request->uri->getSegment(3);
		$this->admin_model->DeleteKategori($id_kategori)->getResultArray();

		// $this->session->set_flashdata('message','Kategori Berhasil Dihapus');
		return redirect()->to("/adminToko/kategori");
	}

	public function kategori_update() {
		$id_kategori = $_POST['id_kategori'];
		$nama_kategori = $_POST['nama_kategori'];

		$cek = $this->admin_model->KategoriSama($nama_kategori)->getResultArray();

		if (count($cek)>0) {
			$success = "1";
		}
		else {
			// $this->session->set_flashdata('berhasil','Kategori Berhasil Disimpan');
			$this->admin_model->KategoriUpdate($id_kategori,$nama_kategori);
			$success="0";
		}

		echo $success;
	}
	//Akhir kategori

	//Awal Brand
	public function brand() {

		$data['data_brand']= $this->admin_model->GetBrand()->getResultArray();
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/brand/index',$data);
        echo view('adminweb/template/footer', $data);
	}

	public function brand_simpan() {
		$nama_brand = $_POST['nama_brand'];
		$cek = $this->admin_model->BrandSama($nama_brand)->getRowArray();
			if ( $cek >0) {
				$success = "1";
			}
			else {
				$this->admin_model->BrandSimpan($nama_brand);
				$success ="0";
			}
		echo $success;
	}

	public function brand_edit() {
		$id_brand = $this->request->uri->getSegment(3);
		$query = $this->admin_model->GetEditBrand($id_brand)->getResultArray();
		foreach ($query as $tampil) {
			$data['id_brand'] = $tampil['id_brand'];
			$data['nama_brand'] = $tampil['nama_brand'];
		}

       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/brand/edit',$data);
        echo view('adminweb/template/footer', $data);
	}

	public function brand_delete() {
		$id_brand = $this->request->uri->getSegment(3);
		$this->admin_model->DeleteBrand($id_brand);

		// $this->session->set_flashdata('message','Brand Berhasil Dihapus');
		return redirect()->to("/adminToko/brand");
		// brand();
	}

	public function brand_update() {
		$id_brand = $_POST['id_brand'];
		$nama_brand = $_POST['nama_brand'];

		$cek = $this->admin_model->BrandSama($nama_brand)->getRowArray();

		if ($cek>0) {
			$success = "1";
		}
		else {
			// $this->session->set_flashdata('berhasil','Brand Berhasil Disimpan');
			$this->admin_model->BrandUpdate($id_brand,$nama_brand);
			$success="0";
		}

		echo $success;
	}
	//Akhir kategori

	//Awal Kota
	public function kota() {

		$data['data_kota']= $this->admin_model->GetKota()->getResultArray();
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/kota/index',$data);
            echo view('adminweb/template/footer', $data);
	}

	public function kota_simpan() {
		$nama_kota = $_POST["nama_kota"];
		$cek = $this->admin_model->KotaSama($nama_kota);

		if ($cek->num_rows()>0) {
			$success = "1";
		}
		else {
			$this->session->set_flashdata('berhasil','Kota Berhasil Disimpan');
			$this->admin_model->KotaSimpan($nama_kota);
			$success="0";
		}

		echo $success;
	}

	public function kota_edit() {
		$id_kota = $this->request->uri->getSegment(3);
		$query = $this->admin_model->GetEditKota($id_kota);
		foreach ($query->result_array() as $tampil) {
			$data['id_kota'] = $tampil['id_kota'];
			$data['nama_kota'] = $tampil['nama_kota'];
		}

       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/kota/edit',$data);
            echo view('adminweb/template/footer', $data);
	}

	public function kota_delete() {
		$id_kota = $this->request->uri->getSegment(3);
		$this->admin_model->DeleteKota($id_kota);

		$this->session->set_flashdata('message','Kota Berhasil Dihapus');
		redirect("adminweb/kota");
	}

	public function kota_update() {
		$id_kota = $_POST['id_kota'];
		$nama_kota = $_POST['nama_kota'];

		$cek = $this->admin_model->KotaSama($nama_kota);

		if ($cek->num_rows()>0) {
			$success = "1";
		}
		else {
			$this->session->set_flashdata('berhasil','Kota Berhasil Disimpan');
			$this->admin_model->KotaUpdate($id_kota,$nama_kota);
			$success="0";
		}

		echo $success;
	}
	//Akhir Kota

	//Awal Bank
	public function bank() {

		$data['data_bank'] = $this->admin_model->GetBank()->getRowArray();
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/bank/index',$data);
            echo view('adminweb/template/footer', $data);

	}

	public function bank_tambah() {
		
       $data = $data + $this->data; echo view('adminweb/bank/add');
        echo view('adminweb/template/header', $data);
        echo view('adminweb/template/footer', $data);
    }

	public function bank_simpan() {

			
			$this->form_validation->set_rules('nama_bank', 'Nama Bank', 'required');
			$this->form_validation->set_rules('nama_pemilik', 'Nama Pemilik', 'required');
			$this->form_validation->set_rules('no_rekening', 'No Rekening', 'required');

			if ($this->form_validation->run() == FALSE)
			{
				
               $data = $data + $this->data; 
                echo view('adminweb/template/header', $data);
                echo view('adminweb/bank/add');
                echo view('adminweb/template/footer', $data);
            }
			else {

				if(empty($_FILES['userfile']['name']))
				{
					
						$in_data['nama_bank'] = $_POST['nama_bank'];
						$in_data['nama_pemilik'] = $_POST['nama_pemilik'];
						$in_data['no_rekening'] = $_POST['no_rekening'];
						$this->db->insert("tbl_bank",$in_data);

					$this->session->set_flashdata('berhasil','Bank Berhasil Disimpan');
					redirect("adminweb/bank");
				}
				else
				{
					$config['upload_path'] = './images/bank/';
					$config['allowed_types']= 'gif|jpg|png|jpeg';
					$config['encrypt_name']	= TRUE;
					$config['remove_spaces']	= TRUE;	
					$config['max_size']     = '3000';
					$config['max_width']  	= '3000';
					$config['max_height']  	= '3000';
					
			 
					$this->load->library('upload', $config);
	 
					if ($this->upload->do_upload("userfile")) {
						$data	 	= $this->upload->data();
			 
						/* PATH */
						$source             = "./images/bank/".$data['file_name'] ;
						$destination_thumb	= "./images/bank/thumb/" ;
						$destination_medium	= "./images/bank/medium/" ;
						// Permission Configuration
						chmod($source, 0777) ;
			 
						/* Resizing Processing */
						// Configuration Of Image Manipulation :: Static
						$this->load->library('image_lib') ;
						$img['image_library'] = 'GD2';
						$img['create_thumb']  = TRUE;
						$img['maintain_ratio']= TRUE;
			 
						/// Limit Width Resize
						$limit_medium   = 800 ;
						$limit_thumb    = 270 ;
			 
						// Size Image Limit was using (LIMIT TOP)
						$limit_use  = $data['image_width'] > $data['image_height'] ? $data['image_width'] : $data['image_height'] ;
			 
						// Percentase Resize
						if ($limit_use > $limit_thumb) {
							$percent_medium = $limit_medium/$limit_use ;
							$percent_thumb  = $limit_thumb/$limit_use ;
						}
			 
						//// Making THUMBNAIL ///////
						$img['width']  = $limit_use > $limit_thumb ?  $data['image_width'] * $percent_thumb : $data['image_width'] ;
						$img['height'] = $limit_use > $limit_thumb ?  $data['image_height'] * $percent_thumb : $data['image_height'] ;
			 
						// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_thumb ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;

						$img['width']   = $limit_use > $limit_medium ?  $data['image_width'] * $percent_medium : $data['image_width'] ;
						$img['height']  = $limit_use > $limit_medium ?  $data['image_height'] * $percent_medium : $data['image_height'] ;
			 
			 			// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_medium ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;
						
						$in_data['nama_bank'] = $_POST['nama_bank'];
						$in_data['nama_pemilik'] = $_POST['nama_pemilik'];
						$in_data['no_rekening'] = $_POST['no_rekening'];
						$in_data['gambar'] = $data['file_name'];
						
						
						$this->db->insert("tbl_bank",$in_data);

				
						
						$this->session->set_flashdata('berhasil','Bank Berhasil Disimpan');
						redirect("adminweb/bank");
						
					}
					else 
					{
                        
                       $data = $data + $this->data; echo view('adminweb/bank/error');
                        echo view('adminweb/template/header', $data);
                        echo view('adminweb/template/footer', $data);
                    }
				}
				
			}

	}

	public function bank_delete() {
		$id_bank = $this->request->uri->getSegment(3);
		$this->admin_model->DeleteBank($id_bank);

		$this->session->set_flashdata('message','Bank Berhasil Dihapus');
		redirect('adminweb/bank');

	}

	public function bank_edit() {
		$id_bank = $this->request->uri->getSegment(3);
		$query = $this->admin_model->GetBankEdit($id_bank);
		foreach ($query->result_array() as $tampil) {
			$data['id_bank'] = $tampil['id_bank'];
			$data['nama_bank'] = $tampil['nama_bank'];
			$data['nama_pemilik'] = $tampil['nama_pemilik'];
			$data['no_rekening'] = $tampil['no_rekening'];
			$data['gambar'] = $tampil['gambar'];
		}
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/bank/edit',$data);
            echo view('adminweb/template/footer', $data);
	}

	public function bank_update() {
		$id['id_bank'] = $_POST["id_bank"];

		if(empty($_FILES['userfile']['name']))
				{
					
						$in_data['nama_bank'] = $_POST['nama_bank'];
						$in_data['nama_pemilik'] = $_POST['nama_pemilik'];
						$in_data['no_rekening'] = $_POST['no_rekening'];
						
						$this->db->update("tbl_bank",$in_data,$id);

					$this->session->set_flashdata('update','Bank Berhasil Diupdate');
					redirect("adminweb/bank");
				}
				else
				{
					$config['upload_path'] = './images/bank/';
					$config['allowed_types']= 'gif|jpg|png|jpeg';
					$config['encrypt_name']	= TRUE;
					$config['remove_spaces']	= TRUE;	
					$config['max_size']     = '3000';
					$config['max_width']  	= '260';
					$config['max_height']  	= '100';
					
			 
					$this->load->library('upload', $config);
	 
					if ($this->upload->do_upload("userfile")) {
						$data	 	= $this->upload->data();
			 
						/* PATH */
						$source             = "./images/bank/".$data['file_name'] ;
						$destination_thumb	= "./images/bank/thumb/" ;
						$destination_medium	= "./images/bank/medium/" ;
			 
						// Permission Configuration
						chmod($source, 0777) ;
			 
						/* Resizing Processing */
						// Configuration Of Image Manipulation :: Static
						$this->load->library('image_lib') ;
						$img['image_library'] = 'GD2';
						$img['create_thumb']  = TRUE;
						$img['maintain_ratio']= TRUE;
			 
						/// Limit Width Resize
						$limit_medium   = 90 ;
						$limit_thumb    = 60 ;
			 
						// Size Image Limit was using (LIMIT TOP)
						$limit_use  = $data['image_width'] > $data['image_height'] ? $data['image_width'] : $data['image_height'] ;
			 
						// Percentase Resize
						if ($limit_use > $limit_thumb) {
							$percent_medium = $limit_medium/$limit_use ;
							$percent_thumb  = $limit_thumb/$limit_use ;
						}
			 
						//// Making THUMBNAIL ///////
						$img['width']  = $limit_use > $limit_thumb ?  $data['image_width'] * $percent_thumb : $data['image_width'] ;
						$img['height'] = $limit_use > $limit_thumb ?  $data['image_height'] * $percent_thumb : $data['image_height'] ;
			 
						// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_thumb ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;
	 
						////// Making MEDIUM /////////////
						$img['width']   = $limit_use > $limit_medium ?  $data['image_width'] * $percent_medium : $data['image_width'] ;
						$img['height']  = $limit_use > $limit_medium ?  $data['image_height'] * $percent_medium : $data['image_height'] ;
			 
						// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_medium ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;
						
						$in_data['nama_bank'] = $_POST['nama_bank'];
						$in_data['nama_pemilik'] = $_POST['nama_pemilik'];
						$in_data['no_rekening'] = $_POST['no_rekening'];
						$in_data['gambar'] = $data['file_name'];
						
						$this->db->update("tbl_bank",$in_data,$id);
				
						
						$this->session->set_flashdata('update','Bank Berhasil Diupdate');
						redirect("adminweb/bank");
						
					}
					else 
					{
                        
                       $data = $data + $this->data; echo view('adminweb/bank/error');
                        echo view('adminweb/template/header', $data);
                        echo view('adminweb/template/footer', $data);
                    }
				}

	}
	//Akhir Bank

	//Awal Tentang Kami
	public function tentangkami() {
		// if ($this->session->userdata("logged_in")!=="") {
            if(true){

			$query = $this->admin_model->GetTentangkami()->getResultArray();
			foreach ($query as $tampil) {
				$data['id_tentangkami']=$tampil['id_tentangkami'];
				$data['judul']=$tampil['judul'];
				$data['deskripsi']=$tampil['deskripsi'];
			}

           $data = $data + $this->data; 
            echo view('adminweb/template/header', $data);
            echo view('adminweb/tentangkami/index',$data);
            echo view('adminweb/template/footer', $data);
		}
		else {
			redirect("adminweb");
		}
	}

	public function tentangkami_simpan() {
		$id_tentangkami = $_POST['id_tentangkami'];
		$judul = $_POST['judul'];
		$deskripsi = $_POST['deskripsi'];

		$this->admin_model->UpdateTentangkami($id_tentangkami,$judul,$deskripsi);
	}
	//Akhir Tentang Kami

	//Awal Cara Belanja
	public function carabelanja() {
		// if ($this->session->userdata("logged_in")!=="") {
            if(true){

			$query = $this->admin_model->GetCarabelanja()->getResultArray();
			foreach ($query as $tampil) {
				$data['id_carabelanja']=$tampil['id_carabelanja'];
				$data['judul']=$tampil['judul'];
				$data['deskripsi']=$tampil['deskripsi'];
			}

           $data = $data + $this->data; 
            echo view('adminweb/template/header', $data);
            echo view('adminweb/carabelanja/index',$data);
            echo view('adminweb/template/footer', $data);
		}
		else {
			redirect("adminweb");
		}
	}

	public function carabelanja_simpan() {
		$id_carabelanja = $_POST['id_carabelanja'];
		$judul = $_POST['judul'];
		$deskripsi = $_POST['deskripsi'];

		$this->admin_model->UpdateCarabelanja($id_carabelanja,$judul,$deskripsi);
	}
	//Akhir Cara Belanja

	//Awal Sambutan
	public function sambutan() {
		// if ($this->session->userdata("logged_in")!=="") {
            if(true){

			$query = $this->admin_model->GetSambutan();
			foreach ($query->result_array() as $tampil) {
				$data['id_sambutan']=$tampil['id_sambutan'];
				$data['judul']=$tampil['judul'];
				$data['deskripsi']=$tampil['deskripsi'];
			}

           $data = $data + $this->data; 
            echo view('adminweb/template/header', $data);
            echo view('adminweb/sambutan/index',$data);
            echo view('adminweb/template/footer', $data);
		}
		else {
			redirect("adminweb");
		}
	}

	public function sambutan_simpan() {
		$id_sambutan = $_POST['id_sambutan'];
		$judul = $_POST['judul'];
		$deskripsi = $_POST['deskripsi'];

		$this->admin_model->UpdateSambutan($id_sambutan,$judul,$deskripsi);
	}
	//Akhir Sambutan

	//Awal Admin
	public function admin() {
		// if($this->session->userdata("logged_in")!=="") {
            if(true){

			$data['data_admin'] = $this->admin_model->Getadmin()->getResultArray();
           $data = $data + $this->data; 
            echo view('adminweb/template/header', $data);
            echo view('adminweb/admin/index',$data);
            echo view('adminweb/template/footer', $data);
		}
		else {
			redirect("adminweb");
		}
	} 

	public function admin_delete() {
		$id = $this->request->uri->getSegment(3);
		$this->admin_model->Deleteadmin($id);

		$this->session->set_flashdata('message','Admin Berhasil Dihapus');
		redirect('adminweb/admin');
	}

	public function admin_tambah() {
        
	   	$data = $this->data; 
	   	echo view('adminweb/template/header', $data);
	   	echo view('adminweb/admin/add');
    	echo view('adminweb/template/footer', $data);
    }

	public function admin_simpan() {			
		if(!$this->validate([
			'nama_admin'  => 'required',
			// 'email'  => 'required',
			// 'username'  => 'required',
			// 'password'  => 'required',
			// 'phone'  => 'required',
			// 'hak_akses'  => 'required'
		])){
			$data = $this->data; 
			echo view('adminweb/template/header', $data);
			echo view('adminweb/admin/add');
			echo view('adminweb/template/footer', $data);
        }else {

			$in_data['nama_admin'] 		= $_POST['nama_admin'];
			$in_data['email'] 			= $_POST['email'];
			$in_data['username'] 		= $_POST['username'];
			$in_data['password'] 		= md5($_POST['password']);
			$in_data['phone'] 			= $_POST['phone'];
			$in_data['hak_akses'] 		= $_POST['hak_akses'];
			$this->db->insert("tbl_admin",$in_data);
			return redirec()->to("/adminToko/admin/");
				
			}

	}

	public function admin_edit() {
		$id = $this->request->uri->getSegment(3);
		$query = $this->admin_model->GetadminEdit($id);
		foreach ($query->result_array() as $tampil) {
			$data['id_admin'] = $tampil['id_admin'];
			$data['nama_admin'] = $tampil['nama_admin'];
			$data['email'] = $tampil['email'];
			$data['username'] = $tampil['username'];
			$data['password'] = $tampil['password'];
			$data['phone'] = $tampil['phone'];
			$data['hak_akses'] = $tampil['hak_akses'];
			
		}
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/admin/edit',$data);
            echo view('adminweb/template/footer', $data);
	}

	public function admin_update() {
		$id['id_admin'] = $_POST["id_admin"];

		if ($_POST['password']!=="") {

			$in_data['nama_admin'] = $_POST['nama_admin'];
			$in_data['email'] = $_POST['email'];
			$in_data['username'] = $_POST['username'];
			$in_data['password'] = md5($_POST['password']);
			$in_data['phone'] = $_POST['phone'];
			$in_data['hak_akses'] = $_POST['hak_akses'];

		}
		else {
			$in_data['nama_admin'] = $_POST['nama_admin'];
			$in_data['email'] = $_POST['email'];
			$in_data['username'] = $_POST['username'];
			$in_data['phone'] = $_POST['phone'];
			$in_data['hak_akses'] = $_POST['hak_akses'];
		}
								
		$this->db->update("tbl_admin",$in_data,$id);

		$this->session->set_flashdata('update','Admin Berhasil Diupdate');
		redirect("adminweb/admin");
		
	}
	//Akhir Admin

	//Awal Jasa Pengirman
	public function jasapengiriman() {

		$data['data_jasapengiriman'] = $this->admin_model->GetJasapengiriman()->getResultArray();
       	$data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/jasapengiriman/index',$data);
            echo view('adminweb/template/footer', $data);

	}

	public function jasapengiriman_tambah() {
        
	   	$data = $this->data; 
		echo view('adminweb/template/header', $data);
	   	echo view('adminweb/jasapengiriman/add');
        echo view('adminweb/template/footer', $data);
    }

	public function jasapengiriman_simpan() {
		if(!$this->validate([
			'nama' => 'required'
		])){	
               	$data =  $this->data; 
                echo view('adminweb/template/header', $data);
                echo view('adminweb/jasapengiriman/add');
                echo view('adminweb/template/footer', $data);
            }
			else {
				if(empty($_FILES['userfile']['name']))
				{
					$in_data['nama'] = $_POST['nama'];
					$this->db->insert("tbl_jasapengiriman",$in_data);
					$this->session->set_flashdata('berhasil','Jasa Pengiriman Berhasil Disimpan');
					redirect("adminweb/jasapengiriman");
				}
				else
				{
					$config['upload_path'] = './images/jasapengiriman/';
					$config['allowed_types']= 'gif|jpg|png|jpeg';
					$config['encrypt_name']	= TRUE;
					$config['remove_spaces']	= TRUE;	
					$config['max_size']     = '3000';
					$config['max_width']  	= '150';
					$config['max_height']  	= '60';

					$this->load->library('upload', $config);
	 
					if ($this->upload->do_upload("userfile")) {
						$data	 	= $this->upload->data();
			 
						/* PATH */
						$source             = "./images/jasapengiriman/".$data['file_name'] ;
						$destination_thumb	= "./images/jasapengiriman/thumb/" ;
						$destination_medium	= "./images/jasapengiriman/medium/" ;
						// Permission Configuration
						chmod($source, 0777) ;
			 
						/* Resizing Processing */
						// Configuration Of Image Manipulation :: Static
						$this->load->library('image_lib') ;
						$img['image_library'] = 'GD2';
						$img['create_thumb']  = TRUE;
						$img['maintain_ratio']= TRUE;
			 
						/// Limit Width Resize
						$limit_medium   = 150 ;
						$limit_thumb    = 60 ;
			 
						// Size Image Limit was using (LIMIT TOP)
						$limit_use  = $data['image_width'] > $data['image_height'] ? $data['image_width'] : $data['image_height'] ;
			 
						// Percentase Resize
						if ($limit_use > $limit_thumb) {
							$percent_medium = $limit_medium/$limit_use ;
							$percent_thumb  = $limit_thumb/$limit_use ;
						}
			 
						//// Making THUMBNAIL ///////
						$img['width']  = $limit_use > $limit_thumb ?  $data['image_width'] * $percent_thumb : $data['image_width'] ;
						$img['height'] = $limit_use > $limit_thumb ?  $data['image_height'] * $percent_thumb : $data['image_height'] ;
			 
						// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_thumb ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;

						$img['width']   = $limit_use > $limit_medium ?  $data['image_width'] * $percent_medium : $data['image_width'] ;
						$img['height']  = $limit_use > $limit_medium ?  $data['image_height'] * $percent_medium : $data['image_height'] ;
			 
			 			// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_medium ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;
						
						$in_data['nama'] = $_POST['nama'];
						$in_data['gambar'] = $data['file_name'];
						
						
						
						$this->db->insert("tbl_jasapengiriman",$in_data);

				
						
						$this->session->set_flashdata('berhasil','Jasa Pengiriman Berhasil Disimpan');
						redirect("adminweb/jasapengiriman");
						
					}
					else 
					{
                        
                       $data = $data + $this->data; echo view('adminweb/jasapengiriman/error');
                        echo view('adminweb/template/header', $data);
                        echo view('adminweb/template/footer', $data);
                    }
				}
				
			}

	}

	public function jasapengiriman_delete() {
		$id_jasapengiriman = $this->request->uri->getSegment(3);
		$this->admin_model->DeleteJasapengiriman($id_jasapengiriman);

		$this->session->set_flashdata('message','Jasa Pengiriman Berhasil Dihapus');
		redirect('adminweb/jasapengiriman');

	}

	public function jasapengiriman_edit() {
		$id_jasapengiriman = $this->request->uri->getSegment(3);
		$query = $this->admin_model->GetJasapengirimanEdit($id_jasapengiriman);
		foreach ($query->result_array() as $tampil) {
			$data['id_jasapengiriman'] = $tampil['id_jasapengiriman'];
			$data['nama'] = $tampil['nama'];
			$data['gambar'] = $tampil['gambar'];
		}
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/jasapengiriman/edit',$data);
            echo view('adminweb/template/footer', $data);
	}

	public function jasapengiriman_update() {
		$id['id_jasapengiriman'] = $_POST["id_jasapengiriman"];

		if(empty($_FILES['userfile']['name']))
				{
					
						$in_data['nama'] = $_POST['nama'];
					
						$this->db->update("tbl_jasapengiriman",$in_data,$id);

					$this->session->set_flashdata('update','Jasa Pengiriman Berhasil Diupdate');
					redirect("adminweb/jasapengiriman");
				}
				else
				{
					$config['upload_path'] = './images/jasapengiriman/';
					$config['allowed_types']= 'gif|jpg|png|jpeg';
					$config['encrypt_name']	= TRUE;
					$config['remove_spaces']	= TRUE;	
					$config['max_size']     = '3000';
					$config['max_width']  	= '150';
					$config['max_height']  	= '60';
					
			 
					$this->load->library('upload', $config);
	 
					if ($this->upload->do_upload("userfile")) {
						$data	 	= $this->upload->data();
			 
						/* PATH */
						$source             = "./images/jasapengiriman/".$data['file_name'] ;
						$destination_thumb	= "./images/jasapengiriman/thumb/" ;
						$destination_medium	= "./images/jasapengiriman/medium/" ;
			 
						// Permission Configuration
						chmod($source, 0777) ;
			 
						/* Resizing Processing */
						// Configuration Of Image Manipulation :: Static
						$this->load->library('image_lib') ;
						$img['image_library'] = 'GD2';
						$img['create_thumb']  = TRUE;
						$img['maintain_ratio']= TRUE;
			 
						/// Limit Width Resize
						$limit_medium   = 800 ;
						$limit_thumb    = 270 ;
			 
						// Size Image Limit was using (LIMIT TOP)
						$limit_use  = $data['image_width'] > $data['image_height'] ? $data['image_width'] : $data['image_height'] ;
			 
						// Percentase Resize
						if ($limit_use > $limit_thumb) {
							$percent_medium = $limit_medium/$limit_use ;
							$percent_thumb  = $limit_thumb/$limit_use ;
						}
			 
						//// Making THUMBNAIL ///////
						$img['width']  = $limit_use > $limit_thumb ?  $data['image_width'] * $percent_thumb : $data['image_width'] ;
						$img['height'] = $limit_use > $limit_thumb ?  $data['image_height'] * $percent_thumb : $data['image_height'] ;
			 
						// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_thumb ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;
	 
						////// Making MEDIUM /////////////
						$img['width']   = $limit_use > $limit_medium ?  $data['image_width'] * $percent_medium : $data['image_width'] ;
						$img['height']  = $limit_use > $limit_medium ?  $data['image_height'] * $percent_medium : $data['image_height'] ;
			 
						// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_medium ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;
						
						$in_data['nama'] = $_POST['nama'];
						$in_data['gambar'] = $data['file_name'];
						
						$this->db->update("tbl_jasapengiriman",$in_data,$id);
				
						
						$this->session->set_flashdata('update','Jasa Pengiriman Berhasil Diupdate');
						redirect("adminweb/jasapengiriman");
						
					}
					else 
					{
                        
                       $data = $data + $this->data; echo view('adminweb/jasapengiriman/error');
                        echo view('adminweb/template/header', $data);
                        echo view('adminweb/template/footer', $data);
                    }
				}

	}
	//Akhir Jasa Pengiriman

	//Awal Produk 
	public function produk () {

		$data['data_produk'] = $this->admin_model->GetProduk()->getResultArray();
		// var_dump($data);
		// die;
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/produk/index',$data);
            echo view('adminweb/template/footer', $data);
	}
	//Akhir Produk

	public function produk_tambah(){
		$data['kode_produk'] = $this->admin_model->GetMaxKodeProduk();
		$data['data_brand'] = $this->admin_model->GetBrand()->getResultArray();
		$data['data_kategori'] = $this->admin_model->GetKategori()->getResultArray();
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/produk/add',$data);
        echo view('adminweb/template/footer', $data);
	}

	public function produk_simpan() {
		// $this->form_validation->set_rules('nama_produk','Nama Produk','required');
		// $this->form_validation->set_rules('brand_id','Brand','required');
		// $this->form_validation->set_rules('kategori_id','Kategori','required');
		// $this->form_validation->set_rules('harga','Harga','required');
		// $this->form_validation->set_rules('stok','Stok','required');
		// $this->form_validation->set_rules('deskripsi','Deskripsi','required');

		// if ($this->form_validation->run()==FALSE) {
			if (false) {

			$data['kode_produk'] = $this->admin_model->GetMaxKodeProduk();
			$data['data_brand'] = $this->admin_model->GetBrand()->getResultArray();
			$data['data_kategori'] = $this->admin_model->GetKategori()->getResultArray();
           $data = $data + $this->data; 
            echo view('adminweb/template/header', $data);
            echo view('adminweb/produk/add',$data);
            echo view('adminweb/template/footer', $data);

		}
		else {

			if(empty($_FILES['userfile']['name']))
				{
					
						$in_data['kode_produk'] = $_POST['kode_produk'];
						$in_data['nama_produk'] = $_POST['nama_produk'];
						$in_data['harga'] = $_POST['harga'];
						$in_data['stok'] = $_POST['stok'];
						$in_data['deskripsi'] = $_POST['deskripsi'];
						$in_data['kategori_id'] = $_POST['kategori_id'];
						// $in_data['brand_id'] = $_POST['brand_id'];
						$brand_id = $_POST['brand_id'];
						// $this->db->insert("tbl_produk",$in_data);
						// $colom_key = [];
						// $colom_val = [];
						// foreach($in_data as $key => $val){
						// 	// var_dump($key);
						// 	// var_dump($val);
						// 	array_push($colom_key,$key);
						// 	array_push($colom_val,$val);
						// }
						// var_dump($colom_key);
						// var_dump($colom_val);
						// die;

						// $this->db->query('INSERT INTO `tbl_produk` ($colom_key) WHERE ($colom_val)');
						$builder = $this->db->table('tbl_produk');
						$builder->insert($in_data);
						// die;

					// $this->session->set_flashdata('berhasil','Produk Berhasil Disimpan');
					return redirect()->to("/adminToko/produk");

				}
				else
				{
					$config['upload_path'] = './images/produk/';
					$config['allowed_types']= 'gif|jpg|png|jpeg';
					$config['encrypt_name']	= TRUE;
					$config['remove_spaces']	= TRUE;	
					$config['max_size']     = '3000';
					$config['max_width']  	= '268';
					$config['max_height']  	= '249';
					
			 
					$this->load->library('upload', $config);
	 
					if ($this->upload->do_upload("userfile")) {
						$data	 	= $this->upload->data();
			 
						/* PATH */
						$source             = "./images/produk/".$data['file_name'] ;
						$destination_thumb	= "./images/produk/thumb/" ;
						$destination_medium	= "./images/produk/medium/" ;
						// Permission Configuration
						chmod($source, 0777) ;
			 
						/* Resizing Processing */
						// Configuration Of Image Manipulation :: Static
						$this->load->library('image_lib') ;
						$img['image_library'] = 'GD2';
						$img['create_thumb']  = TRUE;
						$img['maintain_ratio']= TRUE;
			 
						/// Limit Width Resize
						$limit_medium   = 268 ;
						$limit_thumb    = 249 ;
			 
						// Size Image Limit was using (LIMIT TOP)
						$limit_use  = $data['image_width'] > $data['image_height'] ? $data['image_width'] : $data['image_height'] ;
			 
						// Percentase Resize
						if ($limit_use > $limit_thumb) {
							$percent_medium = $limit_medium/$limit_use ;
							$percent_thumb  = $limit_thumb/$limit_use ;
						}
			 
						//// Making THUMBNAIL ///////
						$img['width']  = $limit_use > $limit_thumb ?  $data['image_width'] * $percent_thumb : $data['image_width'] ;
						$img['height'] = $limit_use > $limit_thumb ?  $data['image_height'] * $percent_thumb : $data['image_height'] ;
			 
						// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_thumb ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;

						$img['width']   = $limit_use > $limit_medium ?  $data['image_width'] * $percent_medium : $data['image_width'] ;
						$img['height']  = $limit_use > $limit_medium ?  $data['image_height'] * $percent_medium : $data['image_height'] ;
			 
			 			// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_medium ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;
						
						$in_data['kode_produk'] = $_POST['kode_produk'];
						$in_data['nama_produk'] = $_POST['nama_produk'];
						$in_data['harga'] = $_POST['harga'];
						$in_data['stok'] = $_POST['stok'];
						$in_data['deskripsi'] = $_POST['deskripsi'];
						$in_data['kategori_id'] = $_POST['kategori_id'];
						$in_data['brand_id'] = $_POST['brand_id'];
						$in_data['gambar'] = $data['file_name'];
						
						
						
						$this->db->insert("tbl_produk",$in_data);

						

				
						
						// $this->session->set_flashdata('berhasil','Produk Berhasil Disimpan');
						return redirect()->to("/adminToko/produk");
						
					}
					else 
					{
                        
                       $data = $data + $this->data; echo view('adminweb/produk/error');
                        echo view('adminweb/template/header', $data);
                        echo view('adminweb/template/footer', $data);
                    }
				}

		}
	}

	public function produk_delete() {
		$id_produk = $this->request->uri->getSegment(3);
		$this->admin_model->DeleteProduk($id_produk);

		// $this->session->set_flashdata('message','Produk Berhasil Dihapus');
		return redirect()->to('/adminToko/produk');

	}

	public function produk_edit() {
		$id_produk = $this->request->uri->getSegment(3);
		$query = $this->admin_model->EditProduk($id_produk)->getResultArray();
		foreach ($query as $tampil) {

			$data['id_produk']= $tampil['id_produk'];
			$data['kode_produk']= $tampil['kode_produk'];
			$data['nama_produk']= $tampil['nama_produk'];
			$data['harga']= $tampil['harga'];
			$data['stok']= $tampil['stok'];
			$data['deskripsi']= $tampil['deskripsi'];
			$data['kategori_id']= $tampil['kategori_id'];
			$data['brand_id']= $tampil['brand_id'];
			
		}
		$data['data_kategori'] = $this->admin_model->GetKategori()->getResultArray();
		$data['data_brand']  = $this->admin_model->GetBrand()->getResultArray();
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/produk/edit',$data);
            echo view('adminweb/template/footer', $data);
	}

	public function produk_update() {
		$id['id_produk'] = $_POST["id_produk"];

		if(empty($_FILES['userfile']['name']))
				{
					
						$in_data['kode_produk'] = $_POST['kode_produk'];
						$in_data['nama_produk'] = $_POST['nama_produk'];
						$in_data['harga'] = $_POST['harga'];
						$in_data['stok'] = $_POST['stok'];
						$in_data['deskripsi'] = $_POST['deskripsi'];
						$in_data['kategori_id'] = $_POST['kategori_id'];
						$in_data['brand_id'] = $_POST['brand_id'];
					
						// $this->db->update("tbl_produk",$in_data,$id);    
						$builder = $this->db->table('tbl_produk');
						$builder->set($in_data);
						// $builder = $db->table('users');
						// $builder->where();
						$builder->update($in_data, ['id_produk' => $id]);

					// $this->session->set_flashdata('update','Produk Berhasil Diupdate');
					return redirect()->to("/adminToko/produk");
				}
				else
				{
					$config['upload_path'] = './images/produk/';
					$config['allowed_types']= 'gif|jpg|png|jpeg';
					$config['encrypt_name']	= TRUE;
					$config['remove_spaces']	= TRUE;	
					$config['max_size']     = '3000';
					$config['max_width']  	= '268';
					$config['max_height']  	= '249';
					
			 
					$this->load->library('upload', $config);
	 
					if ($this->upload->do_upload("userfile")) {
						$data	 	= $this->upload->data();
			 
						/* PATH */
						$source             = "./images/produk/".$data['file_name'] ;
						$destination_thumb	= "./images/produk/thumb/" ;
						$destination_medium	= "./images/produk/medium/" ;
			 
						// Permission Configuration
						chmod($source, 0777) ;
			 
						/* Resizing Processing */
						// Configuration Of Image Manipulation :: Static
						$this->load->library('image_lib') ;
						$img['image_library'] = 'GD2';
						$img['create_thumb']  = TRUE;
						$img['maintain_ratio']= TRUE;
			 
						/// Limit Width Resize
						$limit_medium   = 268 ;
						$limit_thumb    = 249 ;
			 
						// Size Image Limit was using (LIMIT TOP)
						$limit_use  = $data['image_width'] > $data['image_height'] ? $data['image_width'] : $data['image_height'] ;
			 
						// Percentase Resize
						if ($limit_use > $limit_thumb) {
							$percent_medium = $limit_medium/$limit_use ;
							$percent_thumb  = $limit_thumb/$limit_use ;
						}
			 
						//// Making THUMBNAIL ///////
						$img['width']  = $limit_use > $limit_thumb ?  $data['image_width'] * $percent_thumb : $data['image_width'] ;
						$img['height'] = $limit_use > $limit_thumb ?  $data['image_height'] * $percent_thumb : $data['image_height'] ;
			 
						// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_thumb ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;
	 
						////// Making MEDIUM /////////////
						$img['width']   = $limit_use > $limit_medium ?  $data['image_width'] * $percent_medium : $data['image_width'] ;
						$img['height']  = $limit_use > $limit_medium ?  $data['image_height'] * $percent_medium : $data['image_height'] ;
			 
						// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_medium ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;
						
						$in_data['kode_produk'] = $_POST['kode_produk'];
						$in_data['nama_produk'] = $_POST['nama_produk'];
						$in_data['harga'] = $_POST['harga'];
						$in_data['stok'] = $_POST['stok'];
						$in_data['deskripsi'] = $_POST['deskripsi'];
						$in_data['kategori_id'] = $_POST['kategori_id'];
						$in_data['brand_id'] = $_POST['brand_id'];
						$in_data['gambar'] = $data['file_name'];
						
						$this->db->update("tbl_produk",$in_data,$id);
				
						
						$this->session->set_flashdata('update','Produk Berhasil Diupdate');
						redirect("adminweb/produk");
						
					}
					else 
					{
                        
                       $data = $data + $this->data; echo view('adminweb/produk/error');
                        echo view('adminweb/template/header', $data);
                        echo view('adminweb/template/footer', $data);
                    }
				}

	}

	//Akhir Produk

	//Awal Slider 
	public function slider () {

		$data['data_slider'] = $this->admin_model->GetSlider()->getResultArray();
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/slider/index',$data);
            echo view('adminweb/template/footer', $data);
	}
	

	public function slider_tambah(){
	   $data = $this->data; 
	   echo view('adminweb/template/header', $data);
	   echo view('adminweb/slider/add');
        echo view('adminweb/template/footer', $data);
    }

	public function slider_simpan() {
		// $this->form_validation->set_rules('tittle','Tittle','required');
		// $this->form_validation->set_rules('description','Description','required');

		// if ($this->form_validation->run()==FALSE) {
			if (false) {

            
		   $data = $this->data; 
		   	echo view('adminweb/produk/add');
            echo view('adminweb/template/header', $data);
			echo view('adminweb/template/footer', $data);
		}
		else {

			if(empty($_FILES['userfile']['name']))
				{
					
						$in_data['tittle'] = $_POST['tittle'];
						$in_data['description'] = $_POST['description'];
						$in_data['status'] = $_POST['status'];
						$this->db->insert("tbl_slider",$in_data);

					$this->session->set_flashdata('berhasil','Slider Berhasil Disimpan');
					redirect("adminweb/produk");
				}
				else
				{
					$config['upload_path'] = './images/slider/';
					$config['allowed_types']= 'gif|jpg|png|jpeg';
					$config['encrypt_name']	= TRUE;
					$config['remove_spaces']	= TRUE;	
					$config['max_size']     = '3000';
					$config['max_width']  	= '484';
					$config['max_height']  	= '441';
					
			 
					$this->load->library('upload', $config);
	 
					if ($this->upload->do_upload("userfile")) {
						$data	 	= $this->upload->data();
			 
						/* PATH */
						$source             = "./images/slider/".$data['file_name'] ;
						$destination_thumb	= "./images/slider/thumb/" ;
						$destination_medium	= "./images/slider/medium/" ;
						// Permission Configuration
						chmod($source, 0777) ;
			 
						/* Resizing Processing */
						// Configuration Of Image Manipulation :: Static
						$this->load->library('image_lib') ;
						$img['image_library'] = 'GD2';
						$img['create_thumb']  = TRUE;
						$img['maintain_ratio']= TRUE;
			 
						/// Limit Width Resize
						$limit_medium   = 481 ;
						$limit_thumb    = 441 ;
			 
						// Size Image Limit was using (LIMIT TOP)
						$limit_use  = $data['image_width'] > $data['image_height'] ? $data['image_width'] : $data['image_height'] ;
			 
						// Percentase Resize
						if ($limit_use > $limit_thumb) {
							$percent_medium = $limit_medium/$limit_use ;
							$percent_thumb  = $limit_thumb/$limit_use ;
						}
			 
						//// Making THUMBNAIL ///////
						$img['width']  = $limit_use > $limit_thumb ?  $data['image_width'] * $percent_thumb : $data['image_width'] ;
						$img['height'] = $limit_use > $limit_thumb ?  $data['image_height'] * $percent_thumb : $data['image_height'] ;
			 
						// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_thumb ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;

						$img['width']   = $limit_use > $limit_medium ?  $data['image_width'] * $percent_medium : $data['image_width'] ;
						$img['height']  = $limit_use > $limit_medium ?  $data['image_height'] * $percent_medium : $data['image_height'] ;
			 
			 			// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_medium ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;
						
						$in_data['tittle'] = $_POST['tittle'];
						$in_data['description'] = $_POST['description'];
						$in_data['status'] = $_POST['status'];
						$in_data['gambar'] = $data['file_name'];
						
						
						
						$this->db->insert("tbl_slider",$in_data);

						

				
						
						$this->session->set_flashdata('berhasil','Slider Berhasil Disimpan');
						redirect("adminweb/slider");
						
					}
					else 
					{
                        
                       $data = $data + $this->data; echo view('adminweb/slider/error');
                        echo view('adminweb/template/header', $data);
                        echo view('adminweb/template/footer', $data);
                    }
				}

		}
	}

	public function slider_delete() {
		$id_slider = $this->request->uri->getSegment(3);
		$this->admin_model->DeleteSlider($id_slider);

		$this->session->set_flashdata('message','Slider Berhasil Dihapus');
		redirect('adminweb/slider');

	}

	public function slider_edit() {
		$id_slider = $this->request->uri->getSegment(3);
		$query = $this->admin_model->EditSlider($id_slider);
		foreach ($query->result_array() as $tampil) {

			$data['id_slider']= $tampil['id_slider'];
			$data['tittle']= $tampil['tittle'];
			$data['description']= $tampil['description'];
			$data['status']= $tampil['status'];
			
			
		}
		
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/slider/edit',$data);
            echo view('adminweb/template/footer', $data);
	}

	public function slider_update() {
		$id['id_slider'] = $_POST["id_slider"];

		if(empty($_FILES['userfile']['name']))
				{
					
						$in_data['tittle'] = $_POST['tittle'];
						$in_data['description'] = $_POST['description'];
						$in_data['status'] = $_POST['status'];
						
					
						$this->db->update("tbl_slider",$in_data,$id);

					$this->session->set_flashdata('update','Slider Berhasil Diupdate');
					redirect("adminweb/slider");
				}
				else
				{
					$config['upload_path'] = './images/slider/';
					$config['allowed_types']= 'gif|jpg|png|jpeg';
					$config['encrypt_name']	= TRUE;
					$config['remove_spaces']	= TRUE;	
					$config['max_size']     = '3000';
					$config['max_width']  	= '481';
					$config['max_height']  	= '441';
					
			 
					$this->load->library('upload', $config);
	 
					if ($this->upload->do_upload("userfile")) {
						$data	 	= $this->upload->data();
			 
						/* PATH */
						$source             = "./images/slider/".$data['file_name'] ;
						$destination_thumb	= "./images/slider/thumb/" ;
						$destination_medium	= "./images/slider/medium/" ;
			 
						// Permission Configuration
						chmod($source, 0777) ;
			 
						/* Resizing Processing */
						// Configuration Of Image Manipulation :: Static
						$this->load->library('image_lib') ;
						$img['image_library'] = 'GD2';
						$img['create_thumb']  = TRUE;
						$img['maintain_ratio']= TRUE;
			 
						/// Limit Width Resize
						$limit_medium   = 481 ;
						$limit_thumb    = 441 ;
			 
						// Size Image Limit was using (LIMIT TOP)
						$limit_use  = $data['image_width'] > $data['image_height'] ? $data['image_width'] : $data['image_height'] ;
			 
						// Percentase Resize
						if ($limit_use > $limit_thumb) {
							$percent_medium = $limit_medium/$limit_use ;
							$percent_thumb  = $limit_thumb/$limit_use ;
						}
			 
						//// Making THUMBNAIL ///////
						$img['width']  = $limit_use > $limit_thumb ?  $data['image_width'] * $percent_thumb : $data['image_width'] ;
						$img['height'] = $limit_use > $limit_thumb ?  $data['image_height'] * $percent_thumb : $data['image_height'] ;
			 
						// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_thumb ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;
	 
						////// Making MEDIUM /////////////
						$img['width']   = $limit_use > $limit_medium ?  $data['image_width'] * $percent_medium : $data['image_width'] ;
						$img['height']  = $limit_use > $limit_medium ?  $data['image_height'] * $percent_medium : $data['image_height'] ;
			 
						// Configuration Of Image Manipulation :: Dynamic
						$img['thumb_marker'] = '';
						$img['quality']      = '100%' ;
						$img['source_image'] = $source ;
						$img['new_image']    = $destination_medium ;
			 
						// Do Resizing
						$this->image_lib->initialize($img);
						$this->image_lib->resize();
						$this->image_lib->clear() ;
						
						$in_data['tittle'] = $_POST['tittle'];
						$in_data['description'] = $_POST['description'];
						$in_data['status'] = $_POST['status'];
						$in_data['gambar'] = $data['file_name'];
						
						$this->db->update("tbl_slider",$in_data,$id);
				
						
						$this->session->set_flashdata('update','Slider Berhasil Diupdate');
						redirect("adminweb/slider");
						
					}
					else 
					{
                        
                       $data = $data + $this->data; echo view('adminweb/slider/error');
                        echo view('adminweb/template/header', $data);
                        echo view('adminweb/template/footer', $data);
                    }
				}

	}

	//Akhir Slider

	//Awal Buku Tamu
	public function buku_tamu() {

		// if($this->session->userdata("logged_in")!=="") {
            if(true){

			$data['data_buku_tamu'] = $this->admin_model->GetBukuTamu();

           $data = $data + $this->data; 
            echo view('adminweb/template/header', $data);
            echo view('adminweb/buku_tamu/index',$data);
            echo view('adminweb/template/footer', $data);
		}
		else {
			redirect("adminweb");
		}
	}

	public function buku_tamu_hapus() {

		$id = $this->request->uri->getSegment(3);
		$this->admin_model->DeleteBukuTamu($id);
		
		$this->session->set_flashdata('message','Pesan Berhasil Dihapus');
		redirect("adminweb/buku_tamu");
	}

	public function buku_tamu_detail() {

		$id = $this->request->uri->getSegment(3);
		$status ="1";
		$query = $this->admin_model->DetailBukuTamu($id);
		foreach ($query->result_array() as $tampil) {
			$data['id_hubungikami'] = $tampil['id_hubungikami'];
			$data['nama'] = $tampil['nama'];
			$data['email'] = $tampil['email'];
			$data['hp'] = $tampil['hp'];
			$data['alamat'] = $tampil['alamat'];
			$data['pesan'] = $tampil['pesan'];
			$data['tanggal'] = $tampil['tanggal'];
		}

		$this->admin_model->UpdateStatusBukuTamu($status,$id);
		
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/buku_tamu/detail',$data);
            echo view('adminweb/template/footer', $data);
	}

	public function buku_tamu_balas() {
		// if($this->session->userdata("logged_in")!=="") {
            if(true){

			$id = $this->request->uri->getSegment(3);
			$query = $this->admin_model->DetailBukuTamu($id);
			foreach ($query->result_array() as $tampil) {
				$data['id_hubungikami'] = $tampil['id_hubungikami'];
				$data['nama'] = $tampil['nama'];
				$data['email'] = $tampil['email'];
				$data['hp'] = $tampil['hp'];
				$data['alamat'] = $tampil['alamat'];
				$data['pesan'] = $tampil['pesan'];
				$data['tanggal'] = $tampil['tanggal'];
			}

           $data = $data + $this->data; 
            echo view('adminweb/template/header', $data);
            echo view('adminweb/buku_tamu/balas',$data);
            echo view('adminweb/template/footer', $data);
		}
		else {
			redirect("adminweb");
		}
	}

	public function buku_tamu_balas_simpan() {
		$email = $_POST["email"];
		$judul = $_POST["judul"];
		$isi_hubungi_kami_kirim = $_POST["isi_hubungi_kami_kirim"];

		$this->admin_model->SimpanBukuTamuAdd($email,$judul,$isi_hubungi_kami_kirim);

		$this->load->library('email');
		$this->email->from('info@adriano.com', 'Adriano MX Online Shop');
		$this->email->to($email); 	
		$this->email->subject($judul);
		$this->email->message($isi_hubungi_kami_kirim);	
		$this->email->send();
	}


	public function buku_tamu_add() {
		// if($this->session->userdata("logged_in")!=="") {
            if(true){

            $data = $data + $this->data;
                echo view('adminweb/template/header', $data);
                echo view('adminweb/buku_tamu/add');
                echo view('adminweb/template/footer', $data);
            }
		else {
			redirect("adminweb");
		}
	}

	public function buku_tamu_add_simpan() {
		$email = $_POST["email"];
		$judul = $_POST["judul"];
		$isi_hubungi_kami_kirim = $_POST["isi_hubungi_kami_kirim"];

		$this->admin_model->SimpanBukuTamuAdd($email,$judul,$isi_hubungi_kami_kirim);

		$this->load->library('email');
		$this->email->from('info@adriano.com', 'Adriano MX Online Shop');
		$this->email->to($email); 	
		$this->email->subject($judul);
		$this->email->message($isi_hubungi_kami_kirim);	
		$this->email->send();
	}

	public function buku_tamu_kirim() {

		// if($this->session->userdata("logged_in")!=="") {
            if(true){

			$data['data_buku_tamu_kirim'] = $this->admin_model->GetBukuTamuKirim();

           $data = $data + $this->data; 
            echo view('adminweb/template/header', $data);
            echo view('adminweb/buku_tamu/kirim',$data);
            echo view('adminweb/template/footer', $data);
		}
		else {
			redirect("adminweb");
		}
	}

	public function buku_tamu_kirim_hapus() {

		$id = $this->request->uri->getSegment(3);
		$this->admin_model->DeleteBukuTamuKirim($id);
		
		$this->session->set_flashdata('message','Pesan Berhasil Dihapus');
		redirect("adminweb/buku_tamu_kirim");
	}

	public function buku_tamu_kirim_detail() {

		$id = $this->request->uri->getSegment(3);
		$query = $this->admin_model->DetailBukuTamuKirim($id);
		foreach ($query->result_array() as $tampil) {
			$data['id_hubungi_kami_kirim'] = $tampil['id_hubungi_kami_kirim'];
			$data['kepada'] = $tampil['kepada'];
			$data['judul'] = $tampil['judul'];
			$data['isi_hubungi_kami_kirim'] = $tampil['isi_hubungi_kami_kirim'];
			
		}

		
       $data = $data + $this->data; 
        echo view('adminweb/template/header', $data);
        echo view('adminweb/buku_tamu/detail_kirim',$data);
            echo view('adminweb/template/footer', $data);
	}
	//Akhir Buku Tamu


	

}