...
class admin_model extends Model {
	...
	function CekAdminLogin($username,$password) {
		...
		$ceklogin = $this->db->query("select * from tbl_admin where username='$username' ")->getRowArray();
		// var_dump(count($ceklogin));
		
		if ($ceklogin > 0) {
			
			$sess_data['logged_in'] 	= 'LoginOke';
			$sess_data['id_admin']  	= $ceklogin['id_admin'];
			$sess_data['nama_admin']  	= $ceklogin['nama_admin'];
			$sess_data['username']  	= $ceklogin['username'];
			$sess_data['password']  	= $ceklogin['password'];
			$sess_data['email']  		= $ceklogin['email'];
			$sess_data['phone']  		= $ceklogin['phone'];
			$sess_data['hak_akses']  	= $ceklogin['hak_akses'];

			// dd($sess_data['password']);
			if($password == $sess_data['password']){	
	...
	function getCekStatus(){
		return $this->db->query("select count(status) as stts from tbl_hubungikami where status='0'");
	}
	function getCekStatusTransaksi(){
		return $this->db->query("select count(status) as stts from tbl_transaksi_header where status='0'");
	}
	...
	function UpdateLogoTanpaGambar($id_logo) {
		return $this->db->query("update tbl_logo set id_logo='$id_logo' where id_logo='$id_logo' ");
	}
	//Akhir logo
	...
	//Awal Sambutan
	function GetSambutan() {
		return $this->db->query("select * from tbl_sambutan");
	}

	function UpdateSambutan($id_sambutan,$judul,$deskripsi){
		return $this->db->query("update tbl_sambutan set judul='$judul',deskripsi='$deskripsi' where id_sambutan='$id_sambutan'");
	}
	//Akhir Sambutan
	...
	function GetProduk() {
		...
		return $this->db->query("select a.*,b.nama_brand,c.nama_kategori from tbl_produk a LEFT JOIN tbl_brand b on a.brand_id=b.id_brand left join tbl_kategori c on a.kategori_id=c.id_kategori order by a.id_produk desc");
	}
	...
	function getServices() {
		return $this->db2->query("select * from services");
	}
	function getServicesId($id) {
		return $this->db2->query("SELECT * FROM services WHERE id='$id'");

	}
	
	function getProdukGalery() {
		return $this->db2->query("SELECT * FROM img_galery");
	}

	function GetProdukGaleryId($id){
		return $this->db2->query("SELECT * FROM img_galery Where id='$id'");
	}

	function getTanyaJawab(){
		return $this->db2->query("SELECT * FROM tanyajawab ");
	}
	function getTanyaJawabId($id){
		return $this->db2->query("SELECT * FROM tanyajawab Where id='$id'");
	}
}